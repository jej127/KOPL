...
def run_nsmc(args):
    ...
    model = CNN(word_num, args.word_embed_dim, args.cnn_filter_num, args.cnn_kernel_size, args.dropout, pre_embedding=pre_embeddings).to(device)
...
def predict(args, model, test_iterator, test_iterator_oov, criterion, device, model_path):
    ...
    ratios = ['10% ','30% ','50% ','70% ','90% ','50%_','70%_']
    acc_list = [test_acc]
    print('\t-------------------------------------------------------------------------')
    for ratio, iterator in zip(ratios, test_iterator_oov):
        loss, acc = evaluate(model, iterator, criterion, device)
...
if __name__ == '__main__':
    ...
    #args.pretrain_embed_path = f'/mnt/data/oov/nsmc/love_kor{oov}.emb'
