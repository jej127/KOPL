...
parser.add_argument('--pretrain_embed_path', default='/mnt/oov/klue_tc/love_kor.emb')
...
def train(model, iterator, optimizer, criterion, device):
    ...
    for words, labels in iterator:
        ...
        all_preds.append(torch.max(predictions, dim=-1).indices.detach().cpu().numpy())
...
def run_klue_tc(args):
    ...
    test_data_10 = load_sent_by_id('./extrinsic/klue-tc/data/ynat-v1.1_dev_10.json', word_dict)
    test_data_30 = load_sent_by_id('./extrinsic/klue-tc/data/ynat-v1.1_dev_30.json', word_dict)
    test_data_50 = load_sent_by_id('./extrinsic/klue-tc/data/ynat-v1.1_dev_50.json', word_dict)
    test_data_70 = load_sent_by_id('./extrinsic/klue-tc/data/ynat-v1.1_dev_70.json', word_dict)
    test_data_90 = load_sent_by_id('./extrinsic/klue-tc/data/ynat-v1.1_dev_90.json', word_dict)
    ...
    model = CNN(word_num, args.word_embed_dim, args.cnn_filter_num, args.cnn_kernel_size, args.dropout, args.num_class, 
                pre_embedding=pre_embeddings).to(device)
    ...
    for e in range(args.epochs):
        ...
        train_loss, train_f1 = train(model, train_iterator, optimizer, criterion, device)
        valid_loss, valid_f1 = evaluate(model, dev_iterator, criterion, device)
...
def predict(args, model, test_iterator, test_iterator_oov, criterion, device, model_path):
    model.load_state_dict(torch.load(model_path))
    test_loss, test_f1 = evaluate(model, test_iterator, criterion, device)
    print('\t-------------------------------------------------------------')
    ...
    for ratio, iterator in zip(ratios, test_iterator_oov):
        loss, f1 = evaluate(model, iterator, criterion, device)
