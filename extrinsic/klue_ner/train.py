...
def train_model_wochar(dataloader, model, optimizer, batch_num, writer, use_gpu=False):
    model.train()
    for batch in dataloader:
        batch_num += 1
        model.zero_grad()
...
def evaluate_wochar(dataloader, model, word_vocab, label_vocab, pred_file, score_file, eval_script, use_gpu=False):
    model.eval()
    prediction = []
    for batch in dataloader:
        batch_text, seq_length, word_perm_idx = batch['text']
